...
def word2features(sent, i):    
    ...
    features['ptrn[-2:]']=patninfo[-2:]
    features['sylinfo']='/'.join(str(ele) for ele in sylinfo)
    ...
    if i > 0:
        ...
        features['-1:ptrn[-2:]']=patninfo2[-2:]
        features['-1:sylinfo']='/'.join(str(ele) for ele in sylinfo2)
    ...
    if i < len(sent)-1:
        ...
        features['+1:ptrn[-2:]']=patninfo3[-2:]
        features['+1:sylinfo']='/'.join(str(ele) for ele in sylinfo3)
...
def sent2labels(sent):
    #print(sent)
    return [label for token, postag, label, sylinfo, patninfo, stress in sent]

def sent2tokens(sent):
    #print(sent)
    return [token for token, postag, label, sylinfo, patninfo, stress in sent]
    
def sent_replace_slash(lst,sent):
    nsent=[]
    if len(lst)!=len(sent):
...
train_sents=pickle.load(open('/Users/xinyi/UT/gavo/work/180521/train_sents_stcstress.txt','rb'))
recall_sum=0
precision_sum=0
f_measure_sum=0
new_sents=[]
for i in range(10):
    ...
    correct=0
    wrong=0
    slash_in_original_data=0
    slash_in_result=0
    slash_in_both=0
    for example_sent in test_sents:
        prediction=tagger.tag(sent2features(example_sent))
        new_sents.append(sent_replace_slash(prediction,example_sent))
        print(' '.join(sent2tokens(example_sent)))
        print("Predicted:", ' '.join(tagger.tag(sent2features(example_sent))))
        print("Correct:  ", ' '.join(sent2labels(example_sent)))
        for a in range(len(sent2labels(example_sent))):
            if sent2labels(example_sent)[a]=='S':
                slash_in_original_data+=1
                if tagger.tag(sent2features(example_sent))[a]=='S':
                    slash_in_both+=1
            if tagger.tag(sent2features(example_sent))[a]=='S':
                slash_in_result+=1
    recall=float(slash_in_both)/float(slash_in_original_data)
    precision=float(slash_in_both)/float(slash_in_result)
    f_measure=2*recall*precision/(recall+precision)
    print('Result:\nSlash in Original Data =', slash_in_original_data, '\nSlash in Result =', slash_in_result, '\nSlash in Both =', slash_in_both, '\nRecall =', recall, '\nPrecision =', precision, '\nF-measure =', f_measure)
    recall_sum+=recall
...
pickle.dump(new_sents,open('new_sents_stc.txt','wb'))